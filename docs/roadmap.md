...
> **Current cursor (2026-06-30):** Tracker Days 1–5 done — baselines, regression-from-scratch, trees/ensembles, k-means/PCA segmentation, and (Day 5) a reusable leak-free `sklearn` `Pipeline` + `ColumnTransformer` on Titanic (new notebook in `02-tabular-ml-project`): median/most-frequent imputation with a missing-`age` indicator, `StandardScaler`, `OneHotEncoder(handle_unknown="ignore")`, plus `family_size`/`is_alone` feature engineering. The four-model CV reproduced the Day-3 baseline (~0.80–0.82), and the engineered features were an honest no-op on this data (ensembles already extract that signal). **Next:** the deferred **SMOTE / `scale_pos_weight` class-imbalance deep-dive** — reopen `02-tabular-ml-project` on a genuinely skewed dataset and run `scale_pos_weight` vs a SMOTE-in-CV `imblearn` pipeline vs threshold tuning side by side (Pipelines now in hand). Then **Tracker Day 6** — neural-network fundamentals + a NumPy MLP from scratch. (Project 1's optional scaling-pipeline retrofit remains a low-priority ~30-min side task.)
